...
const STATUS_STYLES: Record<string, { label: string; dot: string; bg: string }> = {
  EMITIDA: { label: 'Emitida', dot: 'bg-blue-400', bg: 'bg-blue-500/10 text-blue-400' },
  ENVIADA: { label: 'Enviada', dot: 'bg-amber-400', bg: 'bg-amber-500/10 text-amber-400' },
  ACEPTADA: { label: 'Aceptada', dot: 'bg-emerald-400', bg: 'bg-emerald-500/10 text-emerald-400' },
}

const inputStyles = 'w-full rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:border-gold/50 focus:outline-none focus:ring-1 focus:ring-gold/20 transition-colors'
      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#1a1f2e] shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">
              ...
              <span className="text-gray-500">DNI {worker.dni}</span>
            </p>
          </div>
          <button type="button" onClick={onClose} className="rounded-lg p-2 text-gray-400 hover:bg-white/5 hover:text-white transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
        <div className="space-y-5 p-6">
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold uppercase tracking-wider text-gray-400">Periodo</label>
            <input type="month" value={periodo} onChange={e => setPeriodo(e.target.value)} className={inputStyles} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-400">Horas Extras (S/)</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-400">Bonificaciones (S/)</label>
          ...
          <label className="flex items-center gap-4 cursor-pointer rounded-xl border border-white/10 bg-white/[0.02] px-5 py-4 hover:bg-white/[0.04] transition-colors">
            <input type="checkbox" checked={incluirGratif} onChange={e => setIncluirGratif(e.target.checked)} className="h-4 w-4 rounded border-white/20 bg-white/10 text-gold focus:ring-gold/30" />
            <div>
              <p className="text-sm font-semibold text-white">Incluir Gratificacion</p>
              <p className="text-xs text-gray-500 mt-0.5">Marcar en julio y diciembre</p>
            </div>
          </label>
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-3">Vista previa</p>
        ...
        <div className="flex gap-3 border-t border-white/10 px-6 py-5">
          <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-white/10 py-2.5 text-sm font-medium text-gray-400 hover:bg-white/5 hover:text-white transition-colors">
            Cancelar
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400">Sin boletas registradas</p>
            </div>
          ) : (
            payslips.map(p => {
              const st = STATUS_STYLES[p.status] ?? { label: p.status, dot: 'bg-gray-500', bg: 'bg-gray-500/10 text-gray-400' }
              return (
                <div key={p.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-emerald-400 mt-0.5">{fmt(p.totalIngresos)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gold mt-0.5">{fmt(p.netoPagar)}</p>
                    </div>
                  </div>
                </div>
              )
...
function WorkerGridCard({
  ...
  return (
    <div className="group rounded-xl border border-white/[0.08] bg-[#141824] p-4 flex flex-col transition-all shadow-sm shadow-black/20 hover:shadow-md hover:shadow-black/40 hover:border-white/10 hover:border-white/15">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <button
          ...
          className="rounded-lg p-1.5 text-gray-400 hover:text-gray-300 hover:text-white hover:bg-white/[0.04] hover:bg-white/5 transition-colors opacity-0 group-hover:opacity-100 shrink-0"
          title="Ver historial"
        >
      ...
      <p className="text-[11px] text-gray-500 mb-3">
        DNI: <span className="font-mono text-gray-400">{worker.dni}</span>
      </p>

      {/* Sueldo */}
      <div className="rounded-lg bg-white/[0.03] border border-white/[0.06] px-3 py-2.5 mb-3">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 text-gray-600 mb-0.5">Sueldo Bruto</p>
      ...
      <div className="mt-auto flex items-center justify-between gap-2">
        {hasCurrentMonth ? (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-500 text-emerald-400">
            <CheckCircle2 className="h-3 w-3" />
            Al dia
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2.5 py-1 text-[11px] font-semibold text-amber-600 text-amber-400">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{pendingCount} trabajadores pendientes de boleta</p>
          </div>
        </div>
        <button
...
export default function BoletasPage() {
  ...
  const handleGenerateSuccess = (payslip: Payslip) => {
    if (modalWorker) {
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-gray-400">
          Genera y descarga boletas mensuales para cada trabajador.
          Periodo actual: <span className="font-semibold text-white">{periodoLabel}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-[#141824] p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Generadas</p>
              <p className="text-2xl font-bold text-emerald-400">{generatedCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-[#141824] p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Periodo</p>
      ...
      <div className="rounded-xl border border-blue-500/15 bg-blue-500/[0.05] px-5 py-4">
        <div className="flex items-start gap-3">
          ...
          <p className="text-[13px] text-blue-300/90 leading-relaxed">
            <span className="font-bold text-blue-300">Obligacion legal:</span>{' '}
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-3 pl-11 pr-4 text-sm text-white placeholder-gray-500 focus:border-gold/40 focus:outline-none focus:ring-1 focus:ring-gold/20 transition-colors"
        />
        {loading && (
          <Loader2 className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-gray-500" />
        )}
      </div>
      ...
      {loading && workers.length === 0 ? (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-3 text-sm text-gray-400">Cargando trabajadores...</span>
        </div>
      ) : !loading && workers.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-[#141824]/[0.01] py-20 text-center">
