...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  INACTIVE: { label: 'Inactivo', color: 'bg-white/[0.04] text-gray-300 bg-gray-900/30 text-gray-400' },
  TERMINATED: { label: 'Finalizado', color: 'bg-slate-100 text-slate-700 bg-slate-900/30 text-gray-400' },
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Total</span>
        ...
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Activos</span>
            <ShieldCheck className="w-4 h-4 text-green-500" />
          </div>
          <p className="text-2xl font-bold text-green-600 text-green-400 mt-2">{stats.activos}</p>
        </div>
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">En riesgo</span>
            <AlertTriangle className="w-4 h-4 text-red-500" />
          </div>
          <p className="text-2xl font-bold text-red-600 text-red-400 mt-2">{stats.riesgo}</p>
          <p className="text-[10px] text-gray-400 mt-0.5">Desnaturalización ≥ 60%</p>
        </div>
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Sin retención</span>
            <Receipt className="w-4 h-4 text-blue-500" />
          </div>
          <p className="text-2xl font-bold text-blue-600 text-blue-400 mt-2">{stats.conSuspension}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-white/[0.08] border-slate-600 rounded-lg text-sm bg-[#141824] bg-white/[0.04] text-white text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary"
            />
          </div>
          <select
            value={statusFilter}
            ...
            className="px-3 py-2 border border-white/[0.08] border-slate-600 rounded-lg text-sm bg-[#141824] bg-white/[0.04] text-white text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary"
          >
      ...
      <div className="bg-[#141824] rounded-xl border border-white/[0.08] overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 px-4">
            <Briefcase className="w-12 h-12 text-gray-300 text-gray-600 mx-auto mb-3" />
            <p className="text-sm font-semibold text-gray-300">No hay prestadores registrados</p>
            <p className="text-xs text-gray-500 text-gray-400 mt-1">Agrega tu primer prestador de servicios para empezar.</p>
            <Link
              href="/dashboard/prestadores/nuevo"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-white/[0.02] bg-white/[0.04]/50 text-xs uppercase text-gray-500 text-gray-400">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100 divide-slate-700">
                {providers.map(p => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className="hover:bg-white/[0.02] hover:bg-white/[0.04]/40 transition-colors cursor-pointer"
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        ...
                        {p.area && (
                          <div className="text-xs text-gray-500 text-gray-400">{p.area}</div>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {p.hasSuspensionRetencion ? (
                          <span className="inline-flex items-center gap-1 text-xs text-blue-700 text-blue-400">
                            <ShieldCheck className="w-3.5 h-3.5" />
                            Suspendida
                          </span>
                        ) : p.monthlyAmount > 1500 ? (
                          <span className="text-xs text-amber-700 text-amber-400 font-semibold">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/[0.06] border-white/[0.08]">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-1.5 rounded-md border border-white/[0.08] border-slate-600 text-gray-500 disabled:opacity-40 hover:bg-white/[0.02] hover:bg-white/[0.04]"
              >
