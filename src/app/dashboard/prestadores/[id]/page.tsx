...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  INACTIVE: { label: 'Inactivo', color: 'bg-white/[0.04] text-gray-300' },
  ...
  if (error || !provider) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20">
        ...
        <p className="text-sm font-semibold text-gray-300">{error ?? 'Prestador no encontrado'}</p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              {provider.ruc && (
                <span className="text-xs text-gray-500 text-gray-400">· RUC {provider.ruc}</span>
              )}
              {provider.profession && (
                <span className="text-xs text-gray-500 text-gray-400">· {provider.profession}</span>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-2 border border-red-200 border-red-700 text-red-600 text-red-400 rounded-lg text-sm font-semibold hover:bg-red-50 hover:bg-red-900/20 disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Monto mensual</span>
        ...
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Retención 8%</span>
        ...
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Riesgo reclasif.</span>
        ...
        <div className="bg-[#141824] rounded-xl border border-white/[0.08] p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 text-gray-400 uppercase">Recibos emitidos</span>
            ...
            <button
              ...
              className={cn(
                ...
                tab === t.key
                  ...
                  : 'border-transparent text-gray-500 hover:text-gray-300 hover:text-gray-300'
              )}
            >
              {t.label}
            </button>
        ...
        <div className="p-6">
          {tab === 'info' && (
            <div className="space-y-6">
              {/* Contacto */}
              <div>
                <h3 className="text-xs font-bold text-gray-500 text-gray-400 uppercase mb-3">Contacto</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  ...
                  {provider.address && (
                    <div className="sm:col-span-2 flex items-start gap-2 text-gray-300">
              ...
              {/* Servicio */}
              <div>
                <h3 className="text-xs font-bold text-gray-500 text-gray-400 uppercase mb-3">Servicio contratado</h3>
                <div className="space-y-2 text-sm">
                  <p className="text-white text-gray-100">{provider.servicioDescripcion}</p>
                  {provider.area && (
                    <p className="text-xs text-gray-500 text-gray-400">Área: {provider.area}</p>
                  )}
                  <div className="flex items-center gap-4 text-xs text-gray-500 text-gray-400 pt-2">
              ...
              {/* Retencion */}
              <div>
                <h3 className="text-xs font-bold text-gray-500 text-gray-400 uppercase mb-3">Retención de 4ta categoría</h3>
                {provider.hasSuspensionRetencion ? (
                  <div className="flex items-center gap-2 text-sm text-blue-700 text-blue-400">
                  ...
                  <div className="rounded-lg bg-amber-50 bg-amber-900/20 border border-amber-200 border-amber-700 p-3 text-xs">
                    <p className="font-semibold text-amber-800 text-amber-400">Aplica retención 8% IR 4ta categoría</p>
                    <p className="text-amber-700 text-amber-400 mt-1">
                      Se debe retener S/ {retencion.toFixed(2)} de cada pago mensual. Neto a pagar: S/ {(provider.monthlyAmount - retencion).toFixed(2)}.
                    </p>
                  </div>
                ) : (
                  <p className="text-sm text-gray-500 text-gray-400">No aplica retención (monto ≤ S/ 1,500)</p>
                )}
              </div>

              {provider.notes && (
                <div>
                  <h3 className="text-xs font-bold text-gray-500 text-gray-400 uppercase mb-3">Notas internas</h3>
                  <p className="text-sm text-gray-300 whitespace-pre-wrap">{provider.notes}</p>
                </div>
              )}
            </div>
          )}
          ...
          {tab === 'riesgo' && (
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-xs font-bold text-gray-500 text-gray-400 uppercase mb-3">Indicadores presentes</h3>
                  ...
                  {[
                    ...
                    <li
                      ...
                      className={cn(
                        ...
                        provider[ind.key]
                          ? 'bg-red-50 border-red-200 bg-red-900/20 border-red-700 text-red-700 text-red-400'
                          : 'bg-white/[0.02] border-white/[0.08] bg-white/[0.04]/30 border-slate-600 text-gray-400'
                      )}
                    >
              ...
              {provider.desnaturalizacionRisk >= 60 && (
                <div className="rounded-lg bg-red-50 bg-red-900/20 border border-red-200 border-red-700 p-4">
                  <p className="text-sm font-bold text-red-700 text-red-400 flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4" /> Recomendación
                  </p>
                  <p className="text-xs text-red-600 text-red-400 mt-2">
            ...
            <div>
              {provider.rhInvoices.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm font-semibold text-gray-300">Sin recibos registrados</p>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-white/[0.02] bg-white/[0.04]/50 text-xs uppercase text-gray-500">
                      <tr>
                    ...
                    <tfoot className="border-t-2 border-white/[0.08] border-slate-600">
              ...
              <div className="mt-4 rounded-lg bg-blue-50 bg-blue-900/20 border border-blue-200 border-blue-700 p-3 flex items-start gap-2">
                <Info className="w-4 h-4 text-blue-600 text-blue-400 shrink-0 mt-0.5" />
