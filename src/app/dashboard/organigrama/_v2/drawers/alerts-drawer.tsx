 ...
 * (opcional) sugerencia de tarea. Filtramos por severidad. Cada tarjeta es
 * accionable: "Ver en organigrama" (salta al área) y "Crear tarea".
 */
'use client'

import { useMemo, useState } from 'react'
import { m, AnimatePresence } from 'framer-motion'
import { Bell, X, Loader2, Filter, ArrowUpRight, ListPlus } from 'lucide-react'
    ...
    <AnimatePresence>
      {open && (
        <m.aside
          ...
          className="absolute right-0 top-0 z-20 flex h-full w-[420px] flex-col border-l border-slate-200 bg-white shadow-xl"
          role="dialog"
          aria-label="Alertas del organigrama"
        >
                ...
                {alertsQuery.data && (
                  <p className="text-[11px] text-slate-500">
                    ...
                    <span className="font-semibold tabular-nums">
                      {alertsQuery.data.scoreOrgHealth}
                    </span>
                    /100
                  </p>
                )}
...
function AlertCard({ alert }: { alert: OrgAlertDTO }) {
  ...
  const unitId = alert.affectedUnitIds[0]
    ...
    <li className={`rounded-lg border-l-4 p-3 ${SEVERITY_TONE[alert.severity]}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          {(unitId || alert.suggestedTaskTitle) && (
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              {unitId && (
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white px-2 py-1 text-[11px] font-medium text-slate-700 transition hover:bg-slate-50"
                >
              ...
              {alert.suggestedTaskTitle && (
                <button
                  type="button"
                  onClick={() => openModal('delegated-tasks', { title: alert.suggestedTaskTitle })}
                  className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white px-2 py-1 text-[11px] font-medium text-slate-700 transition hover:bg-slate-50"
                >
