...
const SEVERIDAD_CONFIG = {
  MUY_GRAVE: { label: 'Muy Grave', bg: 'bg-red-100 bg-red-900/30', text: 'text-red-700 text-red-300', border: 'border-red-300 border-red-700', dot: 'bg-red-500' },
  GRAVE:     { label: 'Grave',     bg: 'bg-orange-100 bg-orange-900/30', text: 'text-orange-700 text-orange-300', border: 'border-orange-300 border-orange-700', dot: 'bg-orange-500' },
  LEVE:      { label: 'Leve',      bg: 'bg-yellow-100 bg-yellow-900/20', text: 'text-yellow-700 text-yellow-400', border: 'border-yellow-300 border-yellow-600', dot: 'bg-yellow-500' },
}
...
function UrgenciaBar({ urgencia }: { urgencia: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-1.5 w-24 rounded-full bg-gray-200 bg-white/[0.04] overflow-hidden">
        <div className={cn('h-full rounded-full', color)} style={{ width: `${pct}%` }} />
      </div>
      <span className="text-xs text-gray-500 text-gray-400">{urgencia}/10</span>
    </div>
  )
}
    ...
    <div className={cn('rounded-xl border', cfg.border, riesgo.urgencia >= 8 ? 'ring-1 ring-red-400/40' : '')}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11px] text-gray-400 text-slate-500 font-mono">{riesgo.codigo}</span>
              {riesgo.prioridadFiscalizacion === 1 && (
                <span className="inline-flex items-center gap-0.5 rounded-full bg-red-50 bg-red-900/30 px-2 py-0.5 text-[10px] font-semibold text-red-600 text-red-400">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-4">
              <span className="text-sm font-bold text-red-600 text-red-400">{formatSoles(riesgo.multaEstimadaSoles)}</span>
              <span className="text-xs text-gray-400">→ con subsanación: <span className="font-semibold text-green-600 text-green-400">{formatSoles(riesgo.multaConSubsanacionSoles)}</span></span>
              <span className="text-xs text-green-700 text-green-400 font-medium">Ahorro: {formatSoles(riesgo.ahorroSubsanacion)}</span>
            </div>
          </div>
      ...
      {open && (
        <div className="border-t border-white/[0.08] px-4 py-4 space-y-3 text-sm">
          ...
          {/* Base legal */}
          <div className="flex items-start gap-2 rounded-lg bg-blue-50 bg-blue-900/20 px-3 py-2">
            <Scale className="h-4 w-4 text-blue-600 text-blue-400 mt-0.5 shrink-0" />
            <p className="text-xs text-blue-700 text-blue-300 font-medium">{riesgo.baseLegal}</p>
          </div>

          {/* Trabajadores afectados */}
          {riesgo.trabajadoresAfectados.length > 0 && riesgo.trabajadoresAfectados[0].id !== 'org' && (
            <div>
              <p className="text-xs font-semibold text-gray-500 text-gray-400 uppercase tracking-wide mb-1.5">
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {riesgo.trabajadoresAfectados.slice(0, 10).map((t, i) => (
                  <div key={i} className="flex items-start gap-2 rounded bg-white/[0.02] bg-white/[0.04]/50 px-2.5 py-1.5">
                    <span className="font-medium text-gray-200 text-white shrink-0">{t.nombre}</span>
                    <span className="text-gray-500 text-gray-400 text-xs mt-0.5">{t.detalle}</span>
                  </div>
                ))}
          ...
          {/* Acción inmediata */}
          <div className="rounded-lg bg-green-50 bg-green-900/20 border border-green-200 border-green-800 px-3 py-2">
            <p className="text-xs font-semibold text-green-700 text-green-400 mb-1">
              <CheckCircle2 className="inline h-3.5 w-3.5 mr-1" />
              Acción para subsanar (−90% de multa):
            </p>
            <p className="text-xs text-green-800 text-green-300">{riesgo.accionInmediata}</p>
          </div>

          {/* Resumen de multas */}
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-red-50 bg-red-900/20 px-2 py-2">
              <p className="text-red-600 text-red-400 font-bold text-base">{formatSoles(riesgo.multaEstimadaSoles)}</p>
              <p className="text-red-400 mt-0.5">Sin subsanar</p>
            </div>
            <div className="rounded-lg bg-green-50 bg-green-900/20 px-2 py-2">
              <p className="text-green-600 text-green-400 font-bold text-base">{formatSoles(riesgo.multaConSubsanacionSoles)}</p>
              <p className="text-green-400 mt-0.5">Subsanando hoy</p>
            </div>
            <div className="rounded-lg bg-blue-50 bg-blue-900/20 px-2 py-2">
              <p className="text-blue-600 text-blue-400 font-bold text-base">{formatSoles(riesgo.ahorroSubsanacion)}</p>
              <p className="text-blue-400 mt-0.5">Ahorro (−90%)</p>
            </div>
          </div>
        </div>
      )}
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        ...
        <p className="text-sm text-gray-500 text-gray-400">Analizando riesgos SUNAFIL de tu organización...</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-500 text-gray-400 text-sm">
      ...
      {error && (
        <div className="rounded-xl border border-red-200 border-red-800 bg-red-50 bg-red-900/20 px-4 py-3 text-sm text-red-700 text-red-300">
          {error}
        </div>
      )}
      ...
      {report && (
        <>
          {/* Aviso legal */}
          <div className="rounded-xl border border-blue-200 border-blue-800 bg-blue-50 bg-blue-900/20 px-4 py-3">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <div className="rounded-xl border border-white/[0.08] bg-[#141824] p-4 text-center">
              ...
              <p className="mt-1 text-2xl font-bold text-red-600 text-red-400">
                {formatSoles(report.totalMultaSoles)}
              </p>
              <p className="text-xs text-gray-500 text-gray-400">Multa Total Estimada</p>
            </div>
            <div className="rounded-xl border border-white/[0.08] bg-[#141824] p-4 text-center">
              <CheckCircle2 className="mx-auto h-5 w-5 text-green-500" />
              <p className="mt-1 text-2xl font-bold text-green-600 text-green-400">
                {formatSoles(report.totalMultaConSubsanacionSoles)}
              </p>
              <p className="text-xs text-gray-500 text-gray-400">Subsanando hoy (−90%)</p>
            </div>
            <div className="rounded-xl border border-white/[0.08] bg-[#141824] p-4 text-center">
              <TrendingDown className="mx-auto h-5 w-5 text-blue-500" />
              <p className="mt-1 text-2xl font-bold text-blue-600 text-blue-400">
                {formatSoles(report.ahorroTotalSoles)}
              </p>
              <p className="text-xs text-gray-500 text-gray-400">Ahorro si subsana ya</p>
            </div>
            <div className="rounded-xl border border-white/[0.08] bg-[#141824] p-4 text-center">
              ...
              <p className="text-xs text-gray-500 text-gray-400">Riesgos detectados</p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-white/[0.08] bg-[#141824] p-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg bg-red-50 bg-red-900/20 border border-red-200 border-red-800 p-3 text-center">
                <p className="text-2xl font-bold text-red-600 text-red-400">{report.resumen.muyGraves}</p>
                <p className="text-xs text-red-500 text-red-400 font-medium">Muy Graves</p>
                <p className="text-[10px] text-red-400 mt-0.5">Acción inmediata</p>
              </div>
              <div className="rounded-lg bg-orange-50 bg-orange-900/20 border border-orange-200 border-orange-800 p-3 text-center">
                <p className="text-2xl font-bold text-orange-600 text-orange-400">{report.resumen.graves}</p>
                <p className="text-xs text-orange-500 text-orange-400 font-medium">Graves</p>
                <p className="text-[10px] text-orange-400 mt-0.5">Esta semana</p>
              </div>
              <div className="rounded-lg bg-yellow-50 bg-yellow-900/20 border border-yellow-200 border-yellow-700 p-3 text-center">
                <p className="text-2xl font-bold text-yellow-600 text-yellow-400">{report.resumen.leves}</p>
                <p className="text-xs text-yellow-600 text-yellow-400 font-medium">Leves</p>
                <p className="text-[10px] text-yellow-500 mt-0.5">Planificar</p>
              </div>
            </div>
            {report.resumen.areasMasRiesgosas.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="text-xs text-gray-500 text-gray-400">Áreas más riesgosas:</span>
                {report.resumen.areasMasRiesgosas.map(a => (
                  <span key={a} className="rounded-full bg-white/[0.04] px-2.5 py-0.5 text-xs font-medium text-gray-300 text-gray-200">
          ...
          <div>
            ...
            {filteredRiesgos.length === 0 ? (
              <div className="rounded-xl border border-white/[0.08] bg-[#141824] p-8 text-center">
                <CheckCircle2 className="mx-auto h-10 w-10 text-green-500" />
                <p className="mt-2 font-medium text-gray-300 text-gray-200">
          ...
          {/* Nota final */}
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] bg-white/50 px-4 py-3 text-xs text-gray-500 text-gray-400">
