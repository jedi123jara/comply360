...
import {
  Award, Shield, QrCode, Download, Share2, CheckCircle2, Circle,
  Crown, ExternalLink, Calendar, Clock, Star, BadgeCheck, Copy, Loader2,
  RefreshCw,
} from 'lucide-react'
...
const CERT_NUMBER = 'COMPLY-2026-PE-0847'
const VERIFY_URL = `https://verify.comply360.pe/${CERT_NUMBER}`
    ...
    <div className="space-y-8 pb-12">
      ...
      {!loading && isCertified && certificate ? (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-yellow-400 via-amber-500 to-yellow-600 p-px">
          <div className="rounded-[15px] bg-[#141824] bg-gray-900 p-8">
            <div className="flex flex-col lg:flex-row items-center gap-8">
              ...
              <div className="flex-shrink-0 flex flex-col items-center">
                <div className={`relative w-48 h-48 rounded-full bg-gradient-to-br ${SEAL_COLORS[sealLevel]} flex items-center justify-center shadow-xl shadow-amber-200/40 shadow-amber-900/30`}>
                  <div className="w-40 h-40 rounded-full bg-[#141824] bg-gray-900 flex flex-col items-center justify-center">
                    <Crown className="h-10 w-10 text-amber-500 mb-1" />
                    <span className="text-xs font-semibold text-amber-600 text-amber-400 uppercase tracking-wider">
                      Nivel {SEAL_LABELS[sealLevel]}
                    </span>
                    <span className="text-[10px] text-gray-500 text-gray-400 mt-0.5">
                      COMPLY 360
                    </span>
                  </div>
                </div>
                <p className="mt-3 text-sm font-mono font-semibold text-gray-300">
                  {certNumber}
                </p>
              </div>
              ...
              <div className="flex-1 text-center lg:text-left space-y-4">
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-4 text-sm">
                  <span className="inline-flex items-center gap-1.5 text-gray-600 text-gray-300">
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-lg border border-white/10 border-gray-600 bg-[#141824] bg-gray-800 text-gray-300 text-gray-200 hover:bg-white/[0.02] hover:bg-gray-700 px-4 py-2 text-sm font-medium transition-colors"
                  >
              ...
              <div className="flex-shrink-0 flex flex-col items-center gap-2">
                <div className="w-32 h-32 rounded-xl border-2 border-dashed border-white/10 border-gray-600 flex items-center justify-center bg-white/[0.02] bg-gray-800">
                  <QrCode className="h-16 w-16 text-gray-400 text-gray-500" />
                </div>
                <span className="text-xs text-gray-400 text-gray-500">QR de verificación</span>
              </div>
            </div>
          </div>
        </div>
      ) : !loading ? (
        <div className="rounded-2xl border border-white/[0.08] border-gray-700 bg-[#141824] bg-gray-900 p-8 text-center space-y-4">
          <div className="mx-auto w-20 h-20 rounded-full bg-white/[0.04] bg-gray-800 flex items-center justify-center">
            <Shield className="h-10 w-10 text-gray-400 text-gray-500" />
          </div>
          <h2 className="text-xl font-bold text-white">
            Aún no estás certificado
          </h2>
          <p className="text-sm text-gray-500 text-gray-400 max-w-md mx-auto">
            Completa todos los requisitos para obtener tu sello COMPLY360.
            Llevas {metCount}/{criteria.length} requisitos cumplidos.
          </p>
          <div className="w-full max-w-xs mx-auto bg-gray-200 bg-gray-700 rounded-full h-2.5">
            <div
      ...
      {criteria.length > 0 && (
        <div className="rounded-2xl border border-white/[0.08] border-gray-700 bg-[#141824] bg-gray-900 p-6">
          ...
          <p className="text-sm text-gray-500 text-gray-400 mb-5">
            Estado: <span className="font-semibold text-gray-300 text-gray-200">{metCount}/{criteria.length} requisitos cumplidos</span>
          </p>
          ...
          <ul className="space-y-3">
            {criteria.map((c) => (
              <li
                key={c.id}
                className="flex items-start gap-3 rounded-lg border border-white/[0.06] border-gray-800 bg-white/[0.02] bg-gray-800/50 p-3"
              >
                {c.met ? (
                  <CheckCircle2 className="h-5 w-5 text-emerald-500 mt-0.5 flex-shrink-0" />
                ) : (
                  <Circle className="h-5 w-5 text-gray-300 text-gray-600 mt-0.5 flex-shrink-0" />
                )}
                <div>
                  <span className={`text-sm font-medium ${c.met ? 'text-gray-200' : 'text-gray-500 text-gray-400'}`}>
                    {c.label}
                  </span>
                  <span className="ml-2 text-xs text-gray-400 text-gray-500">
                    {c.value !== null ? `(actual: ${c.value}` : '('}umbral: {c.threshold})
                  </span>
                  {!c.met && (
                    <p className="mt-0.5 text-xs text-amber-600 text-amber-400">{c.description}</p>
                  )}
                </div>
              </li>
            ))}
        ...
        <div className="rounded-2xl border border-white/[0.08] border-gray-700 bg-[#141824] bg-gray-900 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div
                key={item.label}
                className="rounded-lg border border-white/[0.06] border-gray-800 bg-white/[0.02] bg-gray-800/50 p-4"
              >
                <p className="text-xs text-gray-400 text-gray-500 uppercase tracking-wider mb-1">
                  {item.label}
                </p>
      ...
      {/* Public Verification */}
      {isCertified && certificate && (
        <div className="rounded-2xl border border-amber-200 border-amber-800/50 bg-amber-50 bg-amber-950/20 p-6">
          ...
          <p className="text-sm text-gray-400 mb-4">
            ...
            El enlace también está disponible en <code className="text-xs bg-amber-100 bg-amber-900/40 px-1 rounded">/api/certification/verify?code={certificate.verificationCode}</code>
          </p>

          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="flex-1 rounded-lg border border-amber-200 border-amber-800 bg-[#141824] bg-gray-900 px-4 py-2.5 font-mono text-sm text-gray-300 truncate">
              {verifyUrl}
            </div>
            <button
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {benefits.map((b) => (
            <div
              key={b.title}
              className="rounded-2xl border border-white/[0.08] border-gray-700 bg-[#141824] bg-gray-900 p-5 hover:shadow-md hover:shadow-gray-800/40 transition-shadow"
            >
              <div className="w-10 h-10 rounded-lg bg-amber-100 bg-amber-900/30 flex items-center justify-center mb-3">
                <b.icon className="h-5 w-5 text-amber-600 text-amber-400" />
              </div>
              <h4 className="text-sm font-semibold text-gray-200 mb-1">
                {b.title}
              </h4>
              <p className="text-xs text-gray-500 text-gray-400 leading-relaxed">
                {b.desc}
              </p>
            </div>
          ))}
      ...
      <div className="rounded-2xl border border-white/[0.08] border-gray-700 bg-[#141824] bg-gray-900 p-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <Clock className="h-5 w-5 text-gray-400 text-gray-500" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.08] border-gray-700">
                <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500 text-gray-400">
                  Número
                </th>
                <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500 text-gray-400">
                  Nivel
            ...
            <tbody className="divide-y divide-gray-100 divide-gray-800">
              {certificate ? (
                <tr className="hover:bg-white/[0.02] hover:bg-gray-800/50 transition-colors">
                ...
                <tr>
                  <td colSpan={4} className="py-6 text-center text-sm text-gray-400 text-gray-500">
