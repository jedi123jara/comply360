/**
 * Auditor Link público — vista redactada del organigrama firmado.
 *
 * Soporta dos modos de visualización:
 *
 *   - **Modo Inspector SUNAFIL** (default si hay tour disponible) — tour
 *     guiado paso a paso por las áreas críticas de cumplimiento, con
 *     base legal explícita y plan de acción si hay observaciones.
 *
 *   - **Vista clásica** — árbol jerárquico con todos los cargos y comités.
 *     Alternativa para quien quiera el detalle completo del organigrama.
 *
 * El visitante puede alternar entre modos con el toggle del header. El
 * tracking de cada step visitado se registra en AuditLog del cliente.
 */
...
import { AuditorViewSwitcher } from './_components/view-switcher'
...
export default async function AuditorOrgChartPage({ params, searchParams }: PageProps) {
  const [{ token }, sp] = await Promise.all([params, searchParams])
  const data = await fetchPublicChart(token)
  if (!data) notFound()

  // Construir árbol simple para vista clásica
  ...
  // Default = modo guiado si hay tour disponible
  const view = sp.view === 'classic' ? 'classic' : data.guidedTour ? 'guided' : 'classic'
  ...
  return (
    <AuditorViewSwitcher
      ...
      token={token}
      data={data}
      tree={{ roots, childrenByParent, positionsByUnit, rolesByUnit }}
    >
      {/* Vista clásica — fallback render */}
      <div className="min-h-screen bg-slate-50 text-slate-900">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs uppercase tracking-wide text-emerald-700">
                  Auditor Link · Comply360
                </div>
                <h1 className="text-xl font-semibold">{data.org.name}</h1>
                {data.org.ruc && (
                  <div className="text-xs text-slate-500">RUC {data.org.ruc}</div>
                )}
              </div>
            </div>
        ...
        <main className="mx-auto max-w-5xl px-6 py-8">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
            <strong>Documento oficial firmado.</strong> Este organigrama fue generado por
            Comply360 y firmado criptográficamente con SHA-256. Si el contenido fue
            alterado, el hash no coincidiría y el enlace dejaría de validar.{' '}
            <em>Snapshot: {data.snapshotLabel}</em>.
          </div>
          ...
          <footer className="mt-12 border-t border-slate-200 pt-6 text-center text-xs text-slate-500">
            Generado por <strong className="text-emerald-700">Comply360</strong> ·
            Plataforma de compliance laboral peruano. Para verificar autenticidad,
            comparte este enlace y el hash con el responsable.
          </footer>
        </main>
      </div>
    </AuditorViewSwitcher>
...
interface UnitNodeProps {
  ...
  childrenByParent: Map<
    string | null,
    Array<{ id: string; name: string; parentId: string | null; kind: string }>
  >
  positionsByUnit: Map<
    string,
    Array<{
  ...
  rolesByUnit: Map<
    string,
    Array<{
      roleType: keyof typeof COMPLIANCE_ROLES
      workerName: string
      unitId: string | null
      endsAt: string | null
    }>
